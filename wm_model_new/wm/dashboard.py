...
def tab_physical(df: pd.DataFrame) -> str:
    ...
    panel1_series = [
        ...
        ("gen_H", "Households generation", BLUE, "solid", 2),
        ("gen_SC", "Small commercial generation", AMBER, "solid", 2),
        ("gen_LG", "Large generators generation", PURPLE, "solid", 2),
        ...
        ("informal", "Informal leakage", RED, "dot", 2),
    ]
    ...
    # Shared y-axis scale for generation, collection, and the (now real)
    # informal-leakage gap between them. gen_total > Q_HAUL by exactly the
    # informal leakage flow (see the "Informal leakage" trace above) —
    # they no longer coincide now that leakage is reactivated (revised eq. 8).
...
def tab_environment(df: pd.DataFrame) -> str:
    ...
    for col, lbl, color in [("D_climate", "Climate damage", AMBER),
                              ...
                              ("D_water", "Water damage (leachate, UNVERIFIED)", BLUE),
    ...
    # Panel 3 — landfill stock + informal environmental stock
    if "S_LF" in df.columns:
        fig.add_trace(go.Scatter(
            x=df["year"], y=df["S_LF"], name="S_LF (landfill, cumulative)",
    ...
    if "S_ENV" in df.columns:
        fig.add_trace(go.Scatter(
            x=df["year"], y=df["S_ENV"], name="S_ENV (informal leakage, cumulative)",
            mode="lines",
            line=dict(color=AMBER, width=2, dash="dot"),
            hovertemplate="<b>Informal leakage stock</b>: %{y:,.0f} t<extra></extra>",
        ), row=2, col=1)

    # Panel 4 — sorting shares (household-level; theta derived as the residual
    # of the logit: sR+sO+sG+theta = 1 by construction — see forms.sorting_shares)
    ...
    if all(c in df.columns for c in ["sR", "sO", "sG"]):
        theta = (1.0 - df["sR"] - df["sO"] - df["sG"]) * 100
        fig.add_trace(go.Scatter(
            x=df["year"], y=theta, name="Informal leakage (theta)",
            mode="lines+markers",
            line=dict(color=RED, width=2, dash="dot"),
            marker=dict(size=5),
            hovertemplate="<b>Informal leakage (theta)</b>: %{y:.1f}%<extra></extra>",
        ), row=2, col=2)
...
def tab_sankey(df: pd.DataFrame) -> str:
    ...
    nodes = ["Generation", "Haulers", "MRF", "Composting", "WTE", "Landfill",
             "Recovered Material", "Compost Output", "Ash", "Environment (informal)"]
    node_colors = [GREEN, TEAL, BLUE, AMBER, PURPLE, RED, GREEN, AMBER, GREY, RED]
    ...
    for _, row in df.iterrows():
        ...
        _add("Generation", "Environment (informal)", q_informal, "rgba(239,83,80,0.5)")
