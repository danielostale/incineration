...
def _manual_variable_catalogue() -> list[dict]:
    ...
    rows += [
        ...
        _row("gbar_SC", r"\\bar g_{SC}", "Input", "Parameter", "Generation intensity", "ton/economic-activity-unit/yr", "fixed", "eq. 3", "Small-commercial generation intensity, scaled to the exogenous economic-activity index Y (timeseries_exog), not served population.", "Generator behavior", "agents/generator_base.py; agents/small_commercial.py", "assumption", "params_scalar.csv", "gen_total; Q_HAUL", "Provided by modeller", "Higher values increase small-commercial generation.", "Fixed: SC now scales with Y instead of N; still needs local/technology benchmark for gbar_SC itself.", "Parameters_Assumptions"),
        _row("gbar_LG", r"\\bar g_{LG}", "Input", "Parameter", "Generation intensity", "ton/economic-activity-unit/yr", "fixed", "eq. 3", "Large-generator generation intensity, scaled to the exogenous economic-activity index Y (timeseries_exog), not served population.", "Generator behavior", "agents/generator_base.py; agents/large_generators.py", "assumption", "params_scalar.csv", "gen_total; Q_HAUL", "Provided by modeller", "Higher values increase large-generator generation.", "Fixed: LG now scales with Y instead of N; still needs institutional/commercial benchmark for gbar_LG itself.", "Parameters_Assumptions"),
    ...
    rows += [
        ...
        _row("N", r"N_t", "Input", "Exogenous driver", "Demography", "persons", "t", "eq. 3", "Population served by the municipal solid-waste system. Drives Households generation only.", "Generator behavior", "config.py; simulate.py; agents/generator_base.py; agents/households.py", "observed/scenario", "timeseries_exog.csv", "gen_total (via Households)", "Provided by modeller", "Higher values scale household generation proportionally.", "If constant, model assumes no population growth over horizon.", ts),
        _row("Y", r"Y_t", "Input", "Exogenous driver", "Economic activity", "index (economic-activity units)", "t", "eq. 3", "Economic-activity index driving Small Commercial and Large Generators waste generation, in place of served population. Initialized equal to N so introducing this series does not change existing results.", "Generator behavior", "config.py; simulate.py; agents/generator_base.py; agents/small_commercial.py; agents/large_generators.py", "observed/scenario", "timeseries_exog.csv", "gen_total (via SC, LG)", "Provided by modeller", "Higher values scale SC/LG generation proportionally, independent of population.", "Currently a flat placeholder equal to N; replace with real economic-activity data (local GDP, business revenue, employment) for meaningful SC/LG projections.", ts),
