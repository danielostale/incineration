...
uniformly across all generator types in this version of the model). Generation
scales with $\bar g_{LG}$ and is reduced by awareness (eq. 3); they sort
across streams (eq. 8). In the full model, large generators contract directly
with haulers at stream prices $\pi_k^{LG}$; in Stage-0 their formal tonnage
enters the same downstream physical accounting as other generators.
"""
# Bloque 1 — import
